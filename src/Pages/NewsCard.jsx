...
const NewsCard = ({news}) => {
  ...
  return (<div>
    <div className="transition duration-300 rounded-md
          shadow-2xl mb-8">
      <div>
        {/* Card Header */}
        <div className="flex justify-between items-center bg-slate-800 py-2 px-8
        ...
        <img src={image_url} className="w-full rounded-md px-8"/>{" "}
        {
          details.length > 200
            ? (<p className="text-sm text-gray-400 text-justify py-3 px-8">
              {details.slice(0, 200)}{" "}
              <Link to={`/news/${_id}`} className="text-blue-600 underline font-bold text-lg">
                Read More...
              </Link>
            </p>)
            : (<p className="text-sm text-gray-400 text-justify py-3 px-8">
